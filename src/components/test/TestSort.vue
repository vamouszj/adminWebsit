        ...
        <div class="div-sort mt10">
            ...
            <div class="label-div">
                <span class="cursor label" v-for="item in labelList">
                    {{item.paper_label_name}}
                    <span class="gray ml10" @click="removeLabel(item.paper_label_id)">×</span>
                </span>
            </div>
        </div>
    </div>
    ...
    export default {
        ...
        methods: {
            ...
            removeLabel(labelId) {
                ...
                vm.$axios.post('/mapis/test/changeLabelStatus', {labelId: labelId}).then((res) => {
                    if(res.data.state) {
                        vm.$message('标签移除成功');
                        vm.getLabelLists();
                    }
            ...
            addLabel() {
                ...
                vm.$axios.post('/mapis/test/addLabel', {labelName: vm.labelName}).then((res) => {
                    if(res.data.state) {
                        vm.$message('标签添加成功');
                        vm.labelName = '';
                        vm.getLabelLists();
